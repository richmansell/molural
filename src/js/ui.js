...
class UIManager {
    ...
    handleSubmit() {
        ...
        // Crop canvas to visible area (interactive area + bottom display section)
        const cropCanvas = document.createElement('canvas');
        cropCanvas.width = this.canvasManager.canvas.width;
        cropCanvas.height = this.canvasManager.canvas.height;  // Full canvas height (includes bottom non-editable area)
        const cropCtx = cropCanvas.getContext('2d');
        
        // Copy the full visible canvas
        const imageData = this.canvasManager.ctx.getImageData(0, 0, this.canvasManager.canvas.width, this.canvasManager.canvas.height);
